export interface AdminNavItem {
  label: string;
  path: string;
  /** Heroicons outline path 'd' attribute */
  icon: string;
  /** TopNav 드롭다운에서 사용하는 보조 설명 */
  description: string;
}

export interface AdminNavGroup {
  key: 'observability' | 'org' | 'docs-quality' | 'agent-resources';
  label: string;
  /** 사이드바 그룹 아이콘 — 대표 하위 항목 아이콘 재사용 */
  icon: string;
  items: AdminNavItem[];
}

const ICON_DASHBOARD =
  'M3.75 6A2.25 2.25 0 0 1 6 3.75h2.25A2.25 2.25 0 0 1 10.5 6v2.25a2.25 2.25 0 0 1-2.25 2.25H6a2.25 2.25 0 0 1-2.25-2.25V6ZM3.75 15.75A2.25 2.25 0 0 1 6 13.5h2.25a2.25 2.25 0 0 1 2.25 2.25V18a2.25 2.25 0 0 1-2.25 2.25H6A2.25 2.25 0 0 1 3.75 18v-2.25ZM13.5 6a2.25 2.25 0 0 1 2.25-2.25H18A2.25 2.25 0 0 1 20.25 6v2.25A2.25 2.25 0 0 1 18 10.5h-2.25a2.25 2.25 0 0 1-2.25-2.25V6ZM13.5 15.75a2.25 2.25 0 0 1 2.25-2.25H18a2.25 2.25 0 0 1 2.25 2.25V18A2.25 2.25 0 0 1 18 20.25h-2.25A2.25 2.25 0 0 1 13.5 18v-2.25Z';

const ICON_USERS =
  'M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128H5.228A2 2 0 0 1 3.22 17.07a8.632 8.632 0 0 1 2.026-4.564c1.128-1.188 2.714-1.927 4.504-1.927 1.79 0 3.375.739 4.504 1.927M12 9.75a3.75 3.75 0 1 0 0-7.5 3.75 3.75 0 0 0 0 7.5Z';

const ICON_CHUNKING =
  'M3.75 12h16.5m-16.5 3.75h16.5M3.75 19.5h16.5M5.625 4.5h12.75a1.875 1.875 0 0 1 0 3.75H5.625a1.875 1.875 0 0 1 0-3.75Z';

const ICON_WRENCH =
  'M11.42 15.17 17.25 21A2.652 2.652 0 0 0 21 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655 5.653a2.548 2.548 0 1 1-3.586-3.586l6.837-5.63m5.108-.233c.55-.164 1.163-.188 1.743-.14a4.5 4.5 0 0 0 4.486-6.336l-3.276 3.277a3.004 3.004 0 0 1-2.25-2.25l3.276-3.276a4.5 4.5 0 0 0-6.336 4.486c.091 1.076-.071 2.264-.904 2.95l-.102.085m-1.745 1.437L5.909 7.5H4.5L2.25 3.75l1.5-1.5L7.5 4.5v1.409l4.26 4.26m-1.745 1.437 1.745-1.437m6.615 8.206L15.75 15.75M4.867 19.125h.008v.008h-.008v-.008Z';

/** 관리자 영역 그룹 구조 — AdminLayout 사이드바 / AdminSectionTabs / TopNav ADMIN_MENU 공유 단일 소스 */
export const ADMIN_NAV_GROUPS: AdminNavGroup[] = [
  {
    key: 'observability',
    label: '관측',
    icon: ICON_DASHBOARD,
    items: [
      {
        label: '운영 대시보드',
        path: '/admin/dashboard',
        icon: ICON_DASHBOARD,
        description: '문서 적재량·질문 사용량·저장소 상태 통합 현황',
      },
      {
        label: 'Agent Run 관측',
        path: '/admin/agent-runs',
        icon: 'M2.25 18 9 11.25l4.306 4.306a11.95 11.95 0 0 1 5.814-5.518l2.74-1.22m0 0-5.94-2.281m5.94 2.28-2.28 5.941',
        description: '에이전트 실행 이력·사용량 관측',
      },
    ],
  },
  {
    key: 'org',
    label: '조직 관리',
    icon: ICON_USERS,
    items: [
      {
        label: '사용자 관리',
        path: '/admin/users',
        icon: ICON_USERS,
        description: '가입 승인 및 사용자 관리',
      },
      {
        label: '부서 관리',
        path: '/admin/departments',
        icon: 'M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6H15m-1.5 3H15m-1.5 3H15M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21',
        description: '부서 생성·수정·삭제 및 사용자 배정',
      },
    ],
  },
  {
    key: 'docs-quality',
    label: '문서·품질',
    icon: ICON_CHUNKING,
    items: [
      {
        label: '청킹 프로파일',
        path: '/admin/chunking-profiles',
        icon: ICON_CHUNKING,
        description: '조항 청킹 경계 규칙·섹션 요약 LLM 프로파일 관리',
      },
      {
        label: '멀티모달 추출',
        path: '/admin/multimodal',
        icon: 'M2.25 15.75l5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 0 0 1.5-1.5V6a1.5 1.5 0 0 0-1.5-1.5H3.75A1.5 1.5 0 0 0 2.25 6v12a1.5 1.5 0 0 0 1.5 1.5Zm10.5-11.25h.008v.008h-.008V8.25Zm.375 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z',
        description: '그림·차트·이미지형 표 비전 해석 — 모델 선택·비용 가드·미리보기',
      },
      {
        label: '발표자료 양식',
        path: '/admin/blueprints',
        icon: 'M3.75 3v11.25A2.25 2.25 0 0 0 6 16.5h2.25M3.75 3h-1.5m1.5 0h16.5m0 0h1.5m-1.5 0v11.25A2.25 2.25 0 0 1 18 16.5h-2.25m-7.5 0h7.5m-7.5 0-1 3m8.5-3 1 3m0 0 .5 1.5m-.5-1.5h-9.5m0 0-.5 1.5M9 11.25v1.5M12 9v3.75m3-6v6',
        description: 'Golden Sample 에서 스타일·페이지 패턴·서사를 추출해 발표자료 양식으로 저장',
      },
      {
        label: 'RAGAS 평가',
        path: '/admin/ragas',
        icon: 'M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z',
        description: 'RAGAS 기반 답변 품질 평가',
      },
    ],
  },
  {
    key: 'agent-resources',
    label: '에이전트 리소스',
    icon: ICON_WRENCH,
    items: [
      {
        label: 'MCP 서버',
        path: '/admin/mcp-servers',
        icon: 'M5.25 14.25h13.5m-13.5 0a3 3 0 0 1-3-3m3 3a3 3 0 1 0 0 6h13.5a3 3 0 1 0 0-6m-16.5-3a3 3 0 0 1 3-3h13.5a3 3 0 0 1 3 3m-19.5 0a4.5 4.5 0 0 1 .9-2.7L5.737 5.1a3.375 3.375 0 0 1 2.7-1.35h7.126c1.062 0 2.062.5 2.7 1.35l2.587 3.45a4.5 4.5 0 0 1 .9 2.7m0 0a3 3 0 0 1-3 3m0 3h.008v.008h-.008v-.008Zm0-6h.008v.008h-.008v-.008Zm-3 6h.008v.008h-.008v-.008Zm0-6h.008v.008h-.008v-.008Z',
        description: 'MCP 서버 등록·수정·삭제 및 연결 테스트',
      },
      {
        label: '도구 관리',
        path: '/admin/tools',
        icon: ICON_WRENCH,
        description: '도구 카탈로그 조회·빌트인 도구 지정',
      },
      {
        label: '미들웨어 관리',
        path: '/admin/middleware',
        icon: ICON_WRENCH,
        description: '실행 미들웨어 빌트인·강제 지정 및 기본 설정 관리',
      },
      {
        label: 'Skill 관리',
        path: '/admin/skills',
        icon: ICON_WRENCH,
        description: '재사용 Skill(지시문·스크립트) 생성·수정·삭제·Fork',
      },
      {
        label: 'LLM 모델',
        path: '/admin/llm-models',
        icon: 'M8.25 3v1.5M4.5 8.25H3m18 0h-1.5M4.5 12H3m18 0h-1.5m-15 3.75H3m18 0h-1.5M8.25 19.5V21M12 3v1.5m0 15V21m3.75-18v1.5m0 15V21m-9-1.5h10.5a2.25 2.25 0 0 0 2.25-2.25V6.75a2.25 2.25 0 0 0-2.25-2.25H6.75A2.25 2.25 0 0 0 4.5 6.75v10.5a2.25 2.25 0 0 0 2.25 2.25Zm.75-12h9v9h-9v-9Z',
        description: 'LLM 모델 등록·수정·가격·비활성화 관리',
      },
      {
        label: '위키 관리',
        path: '/admin/wiki',
        icon: 'M12 6.042A8.967 8.967 0 0 0 6 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 0 1 6 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 0 1 6-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0 0 18 18a8.967 8.967 0 0 0-6 2.292m0-14.25v14.25',
        description: '에이전트 지식 정제 실행·위키 승인 거버넌스',
      },
    ],
  },
];

/** 전체 항목 flatten — path 기반 소비처(TopNav 활성 매칭 등) 하위 호환용 */
export const ADMIN_NAV_ITEMS: AdminNavItem[] = ADMIN_NAV_GROUPS.flatMap((g) => g.items);

/** 메인 앱에서 관리자 영역 진입 시 첫 페이지 (관측 그룹 첫 탭) */
export const ADMIN_ENTRY_PATH = ADMIN_NAV_GROUPS[0].items[0].path;

/** 항목 활성 매칭: exact 또는 하위 경로 (예: /admin/agent-runs/:runId) */
export const isAdminItemActive = (item: AdminNavItem, pathname: string): boolean =>
  pathname === item.path || pathname.startsWith(`${item.path}/`);

/** 현재 경로가 속한 그룹 (admin 영역 밖 경로면 undefined) */
export const findAdminGroupByPath = (pathname: string): AdminNavGroup | undefined =>
  ADMIN_NAV_GROUPS.find((g) => g.items.some((item) => isAdminItemActive(item, pathname)));
