...
import { useKbSearchHistory } from '@/hooks/useKnowledgeBases';
...
/** KB 단위 검색 히스토리 (kb-retrieval-test D9).
 *  컬렉션 SearchHistoryPanel과 동일 UI — 데이터 훅만 KB 전용.
 *  (컬렉션 컴포넌트는 무변경 원칙, 통합은 후속 후보) */
const KbSearchHistoryPanel = ({ kbId, onApply }: KbSearchHistoryPanelProps) => {
  const [isOpen, setIsOpen] = useState(false);
  const historyQuery = useKbSearchHistory(kbId, { limit: 10 });
...
export default KbSearchHistoryPanel;
