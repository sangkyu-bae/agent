    ...
    items: [
      {
        ...
        description: '공개 에이전트를 탐색하고 구독·포크합니다',
      },
      {
        label: '에이전트 만들기',
        path: '/agent-builder',
...
const TopNav = () => {
  ...
  const menus = user?.role === 'admin' ? [...NAV_MENUS, ADMIN_MENU] : NAV_MENUS;
          ...
          <div className="relative ml-2">
            ...
            {userMenuOpen && (
              <div className="absolute right-0 top-full mt-1.5 w-52 overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-xl shadow-zinc-200/60">
                ...
                <div className="p-1.5">
                  <button
                    onClick={() => { navigate('/usage'); setUserMenuOpen(false); }}
                    className="flex w-full items-center gap-2.5 rounded-xl px-3.5 py-2.5 text-left text-[13.5px] font-medium text-zinc-600 transition-all hover:bg-violet-50 hover:text-violet-700"
                  >
