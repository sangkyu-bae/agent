...
| 스킬 | 설명 | 커버 파일 패턴 |
|------|------|---------------|
| `verify-architecture` | DDD 레이어 의존성 규칙 준수 검증 | `src/domain/**`, `src/application/**`, `src/api/routes/**`, `src/infrastructure/**` |
| `verify-logging` | LOG-001 로깅 규칙 준수 검증 | `src/**/*.py` (tests/ 제외) |
| `verify-tdd` | 프로덕션 모듈 테스트 파일 존재 여부 검증 | `src/**/*.py`, `tests/**/*.py` |
| `verify-implementation` | 모든 verify 스킬 통합 실행 및 보고서 생성 | `src/**/*.py`, `tests/**/*.py` |
