...
_TOOL_USAGE_NORM = (
    ...
    # Design Ref: worker-capability-denial-guard §4.6 (D-08) / Plan SC: FR-01
    # 실측(런 031564e4): 워커는 대화 전문을 받으므로 task 밖 질문("본문을 읽을
    # 수 있나요?")에 자기 도구 범위만 보고 답했고, 그 '불가' 주장이 supervisor
    # 결정을 오염시켰다. 위 규범은 '단정하지 말라'만 있고 '답하지 말라'가 없었다.
    # 전달 의무 뒤에 두고, 대상을 '에이전트 능력·범위 질문'으로 한정한다 —
    # 'A라고만 하라'류 절대 프레이밍 금지(D-08).
    "[현재 작업]에 적힌 일만 수행하세요. 대화에 다른 질문이 있어도, 특히 이 에이전트가\n"
    "무엇을 볼 수 있는지·할 수 있는지를 묻는 질문에는 답하지 마세요 — 그 판단은\n"
    "상위 노드가 전체 워커 목록으로 합니다.\n"
)
