...
| **LLM-MODEL-REG-001** | **docs/02-design/features/llm-model-registry.design.md** | **LLM 모델 레지스트리 (llm_model 테이블 신규 + agent_definition.model_name → llm_model_id FK 마이그레이션, CRUD API /api/v1/llm-models 5개, provider/model_name/display_name/api_key_env/is_default 관리, 시드 데이터 3개, AGENT-004/005/006 모델 로딩 DB 연동)** |
